...
public class RSFontSystem extends DrawingArea {
	...
	public RSFontSystem(boolean TypeFont, String s, CacheArchive archive) {
		try {
			...
			stream_1.pos = stream.readShort() + 4;
	      ...
	      else
	      {
	            ...
	            if (startIndex == -1)
	        {
	          ...
	          if (character != 32)
	          {
	            if (transparency == 256)
	            {
	              if (textShadowColor != -1) {
	                drawCharacter(character, drawX + this.characterDrawXOffsets[character] + 1, 
	                  drawY + this.characterDrawYOffsets[character] + 1, width, height, textShadowColor, 
	                  true);
	              }
	              drawCharacter(character, drawX + this.characterDrawXOffsets[character], 
	                drawY + this.characterDrawYOffsets[character], width, height, textColor, false);
	            }
	            else
	            {
	              if (textShadowColor != -1) {
	                drawTransparentCharacter(character, drawX + this.characterDrawXOffsets[character] + 1, 
	                  drawY + this.characterDrawYOffsets[character] + 1, width, height, textShadowColor, 
	                  transparency, true);
	              }
	              drawTransparentCharacter(character, drawX + this.characterDrawXOffsets[character], 
	                drawY + this.characterDrawYOffsets[character], width, height, textColor, transparency, 
	                false);
	            }
	          }
	          else if (anInt4178 > 0)
	          ...
	          if (strikethroughColor != -1) {
	              ...
	              drawY + (int)(this.baseCharacterHeight * 0.7D), lineWidth, 
	              strikethroughColor);
	          }
	          if (underlineColor != -1) {
	...
	public int getTextWidth1(String string) {
		if (string == null) {
			return 0;
		}
		int startIndex = -1;
	...
	public void drawBasicString(String string, int drawX, int drawY, boolean handleOldSyntax) {
	      ...
	      for(int currentCharacter = 0; currentCharacter < string.length(); ++currentCharacter) {
	         ...
	         if (character == '<') {
	                  ...
	                  if (!effectString.equals(aRSString_4169)) {
	                     if (!effectString.startsWith(startImage)) {
	                        if (effectString.startsWith(startClanImage)) {
	                           try {
	                              offsetY = Integer.valueOf(effectString.substring(5));
	                              Sprite icon = GameClient.cacheSprite[offsetY];
	                     ...
	                     try {
	                        offsetY = 0;
	                        imageId = Integer.valueOf(effectString.substring(4));
	                        ...
	                        Sprite icon = GameClient.cacheSprite[imageId];
	                        int iconModY = icon.myHeight;
	                        if (transparency == 256) {
	            ...
	            if (startIndex == -1) {
	               int width = this.characterWidths[character];
	               offsetY = this.characterHeights[character];
	               if (character != ' ') {
	                  if (transparency == 256) {
	                     if (textShadowColor != -1) {
	                        this.drawCharacter(character, drawX + this.characterDrawXOffsets[character] + 1, drawY + this.characterDrawYOffsets[character] + 1, width, offsetY, textShadowColor, true);
	                     }

	                     this.drawCharacter(character, drawX + this.characterDrawXOffsets[character], drawY + this.characterDrawYOffsets[character], width, offsetY, textColor, false);
	                  } else {
	                     if (textShadowColor != -1) {
	                        this.drawTransparentCharacter(character, drawX + this.characterDrawXOffsets[character] + 1, drawY + this.characterDrawYOffsets[character] + 1, width, offsetY, textShadowColor, transparency, true);
	                     }

	                     this.drawTransparentCharacter(character, drawX + this.characterDrawXOffsets[character], drawY + this.characterDrawYOffsets[character], width, offsetY, textColor, transparency, false);
	                  }
	               } else if (anInt4178 > 0) {
	                  anInt4175 += anInt4178;
	                  drawX += anInt4175 >> 8;
	               ...
	               imageId = this.characterScreenWidths[character];
	               if (strikethroughColor != -1) {
	                  DrawingArea.drawHorizontalLine(drawY + (int)((double)this.baseCharacterHeight * 0.7D), strikethroughColor, imageId, drawX);
	               }

	               if (underlineColor != -1) {
	                  DrawingArea.drawHorizontalLine(drawY + this.baseCharacterHeight, underlineColor, imageId, drawX);
	               }
