...
public final class IndexedImage extends DrawingArea {
    ...
    public IndexedImage(CacheArchive streamLoader, String s, int i, boolean textures) {
        Stream stream = new Stream(streamLoader.getDataForName(s + ".dat"));
        Stream stream_1 = new Stream(streamLoader.getDataForName("index.dat"));
        stream_1.pos = textures ? stream.readInt() : stream.readUnsignedWord();
        libWidth = stream_1.readUnsignedWord();
        libHeight = stream_1.readUnsignedWord();
        int count = textures ? stream_1.readUnsignedWord() : stream_1.readUnsignedByte();
        ...
        for (int l = 0; l < i; l++) {
            stream_1.pos += 2;
            stream.pos += stream_1.readUnsignedWord() * stream_1.readUnsignedWord();
            stream_1.pos++;
        }
        ...
        if (type == 0) {
            for (int k1 = 0; k1 < j1; k1++)
                imgPixels[k1] = textures ? stream.readUnsignedWord() : stream.readSignedByte();

            return;
        }
        if (type == 1) { // actually i need to add the new particle decoder to ur client sec
            for (int l1 = 0; l1 < imgWidth; l1++) {
                for (int i2 = 0; i2 < imgHeight; i2++)
                    imgPixels[l1 + i2 * imgWidth] = textures ? stream.readUnsignedWord() : stream.readSignedByte();
    ...
    public void reduceSetOffset() {
        libWidth /= 2;
        libHeight /= 2;
        int pixelBuffer[] = new int[libWidth * libHeight];
        int i = 0;
    ...
    public void plotScale(int pixels[], int imgPixels[], int palette[], int i, int j, int k, int l,
    ...
    private void drawPixels(int i, int ai[], int abyte0[], int j, int k, int l,
