...
WindowListener {
	...
	public void rebuildFrame(boolean undecorated, int width, int height,
		...
		boolean webc = GameClient.webclient && Jframe.frame == null;
		if(webc) {
			if(mainFrame != null)
				mainFrame.dispose();
			mainFrame = null;
		...
		if (!webc) {
			...
			if(GameClient.instance instanceof Jframe) {
				
				Jframe frame = (Jframe) GameClient.instance;
	...
	final void createClientFrame(int w, int h) {
		...
		myWidth = w;
		myHeight = h;
		mainFrame = new RSFrame(this, myWidth, myHeight, GameClient.clientSize == 2, GameClient.clientSize == 1);
	...
	public void mouseWheelMoved(MouseWheelEvent event) {
		...
		if (mouseX > 0 && mouseX < 512
				&& mouseY > GameClient.getClient().clientHeight - 165
				&& mouseY < GameClient.getClient().clientHeight - 25) {
			...
			if(zoom && GameClient.openInterfaceID == -1) {
				...
				if(GameClient.clientZoom < -630 && GameClient.clientSize > 0)
					GameClient.clientZoom = -630;
				else if(GameClient.clientZoom < -790 && GameClient.clientSize == 0)
					GameClient.clientZoom = -790;
	...
	public boolean handleInterfaceScrolling(MouseWheelEvent event) {
		...
		int tabInterfaceID = GameClient.getClient().tabInterfaceIDs[GameClient.getClient().tabID];
		if(tabInterfaceID == 11000)
			tabInterfaceID = 1151;
		if (tabInterfaceID != -1) {
			RSInterface tab = RSInterface.interfaceCache[tabInterfaceID];
			offsetX = GameClient.getClient().clientSize == 0 ? GameClient.getClient().clientWidth - 218
					: (GameClient.getClient().clientSize == 0 ? 28 : GameClient
							.getClient().clientWidth - 197);
			offsetY = GameClient.getClient().clientSize == 0 ? GameClient.getClient().clientHeight - 298
					: (GameClient.getClient().clientSize == 0 ? 37 : GameClient
							.getClient().clientHeight
							- (GameClient.getClient().clientWidth >= 900 ? 37 : 74)
							- 267);
		...
		if (GameClient.openInterfaceID != -1) {
			...
			offsetX = GameClient.getClient().clientSize == 0 ? 4 : (GameClient
					.getClient().clientWidth / 2) - 256;
			offsetY = GameClient.getClient().clientSize == 0 ? 4 : (GameClient
	...
	public final void keyPressed(KeyEvent keyevent) {
		...
		if (i == 17) {
		      ...
		      GameClient.stream.writeWordBigEndian(command.length() - 1);
		      GameClient.stream.writeString(command.substring(2));
		} 
		if (hotKey == 508) {
