...
public class MysteryBox {
	...
	public void spin() {
		if (GameClient.instance.openInterfaceID != INTERFACE_ID || spinClick == false) {
			return;
		}
	...
	private void spinComplete() {
		...
		GameClient.instance.stream.createFrame(145);
		GameClient.instance.stream.writeUnsignedWordA(696969);
		GameClient.instance.stream.writeUnsignedWordA(0);
		GameClient.instance.stream.writeUnsignedWordA(0);
	}

	public boolean handledPacket34(int frame) {
		if (GameClient.instance.openInterfaceID != INTERFACE_ID) {
			return false;
		}

		RSInterface items = RSInterface.interfaceCache[frame];
		while (GameClient.instance.inStream.pos < GameClient.instance.pktSize) {
			int amount = GameClient.instance.inStream.method422();
			int itemId = GameClient.instance.inStream.readUnsignedWord();
			GameClient.instance.inStream.readUnsignedByte();
			int slot = GameClient.instance.inStream.readDWord();
