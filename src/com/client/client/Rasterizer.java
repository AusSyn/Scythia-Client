...
public final class Rasterizer extends DrawingArea {
	...
	public static void loadTextures(CacheArchive streamLoader) {
        anInt1473 = 0;
        for (int index = 0; index < textureAmount; index++) { // kk sec lemme send u my updated texture editor bet
            try {
                textures[index] = new IndexedImage(streamLoader, String.valueOf(index), 0, true);
                textures[index].setOffset();
                anInt1473++;
            } catch (Exception ex) {
				System.out.println("Failed to load Texture " + index);
            	ex.printStackTrace();
			}
        }
    }
    ...
    private static int[][] method371(int texture) {
		...
		IndexedImage background = textures[texture];
	...
	public static void drawMaterializedTriangle(int y1, int y2, int y3, int x1, int x2, int x3, int hsl1, int hsl2, int hsl3, int tx1, int tx2, int tx3, int ty1, int ty2, int ty3, int tz1, int tz2, int tz3, int tex, int z1, int z2, int z3) {
		...
		if (!GameClient.getOption("hd_tex") || Texture.get(tex) == null) {
	...
	public static CacheArchive textureLoader;
	...
	private static void drawTexturedScanline(int dest[], int src[], int offset, int x1, int x2, int hsl1, int hsl2, int t1, int t2, int t3, int t4, int t5, int t6, int z1, int z2) {
		...
		if(aBoolean1463) {
			while(n-- > 0) {
				int rgb;
				int l;
				rgb = src[texelPos((v & 0x3f80) + (u >> 7)) & (repeatTexture ? 16383 : (v & 0x3f80) + (u >> 7))];
				l = hsl1 >> 16;
		...
		while(n-- > 0) {
			int i9;
			int l;
			if((i9 = src[texelPos((v & 0x3f80) + (u >> 7)) & (repeatTexture ? 16383 : (v & 0x3f80) + (u >> 7))]) != 0) {
				l = hsl1 >> 16;
		...
		for(int l3 = x2 - x1 & 7; l3-- > 0;) {
			int j9;
			int l;
			if((j9 = src[texelPos((v & 0x3f80) + (u >> 7)) & (repeatTexture ? 16383 : (v & 0x3f80) + (u >> 7))]) != 0) {
				l = hsl1 >> 16;
