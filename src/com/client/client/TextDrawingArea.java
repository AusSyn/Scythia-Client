...
public final class TextDrawingArea extends DrawingArea {
	...
	public TextDrawingArea(boolean flag, String s, CacheArchive streamLoader) {
		try {
			...
			stream_1.pos = stream.readUnsignedWord() + 4;
	...
	public void drawRegularText(String text, int x, int y, int color, boolean shadowed) {
	   ...
	   public void drawTextShadowCenter(final String text, int x, final int rgb, int y) {
	        this.method385(0, text, y, x - this.method384(text) / 2);
	        --x;
	        --y;
	        this.method385(rgb, text, y, x - this.method384(text) / 2);
	    }
	    public void drawTextShadow2(final String text, int x, final int rgb, int y) {
	        this.method385(0, text, y, x);
	        --x;
	        --y;
	        this.method385(rgb, text, y, x);
	    }
