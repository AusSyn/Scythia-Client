...
public final class Player extends Entity {
    ...
    public int[][] modifiedColors = new int[14][];
        ...
        if (super.anInt1520 != -1 && super.currentAnim != -1) {
            ...
            if (spotAnim.animation != null) {
                if (FrameReader.animationlist[Integer.parseInt(Integer.toHexString(spotAnim.animation.frameIDs[0])
                        .substring(0, Integer.toHexString(spotAnim.animation.frameIDs[0])
    ...
    public void updatePlayerAppearance(Stream stream) {
        ...
        for (int i = 0; i < cosmeticEquipment.length; i++) {
            ...
            cosmeticEquipment[i] = id > 0 ? id + 512 : 0;
        }

        System.out.println("Equipment: " + Arrays.toString(equipment));
        ...
        if (cosmeticEquipment != null) {
            for (int i = 0; i < equipment.length; i++) {
                if (cosmeticEquipment[i] > 0) {
                    equipment[i] = cosmeticEquipment[i];
                }
            }
        }
        ...
        aLong1718 += equipment[12] * 34;
        aLong1718 += equipment[13] * 65;
    ...
    private int[] cosmeticEquipment = new int[14];
    ...
    public Model method452() {
        ...
        if (model_1 == null) {
            ...
            for (int bodyPartId = 0; bodyPartId < 14; bodyPartId++) {
                ...
                if ((partId - 256) != 846) {
                    ...
                    if (partId >= 256 && partId < 512 && !IDK.cache[partId - 256].bodyModelIsFetched())
                        fetchModels = true;
                    if (partId >= 512 && !ItemDefinition.forID(partId - 512)
                            .equipModelFetched(myGender))
                        fetchModels = true;
        ...
        if (model_1 == null) {
            Model bodyPartModels[] = new Model[14];
            int j2 = 0;
            for (int currentPart = 0; currentPart < 14; currentPart++) {
            ...
            for (int j3 = 0; j3 < 5; j3++)
                if (anIntArray1700[j3] != 0) {
                    model_1.recolour(GameClient.anIntArrayArray1003[j3][0], GameClient.anIntArrayArray1003[j3][anIntArray1700[j3]]);
                    if (j3 == 1)
    ...
    public Model getPlayerModel() {
        ...
        for (int j1 = 0; j1 < 5; j1++)
            if (anIntArray1700[j1] != 0) {
                model.recolour(GameClient.anIntArrayArray1003[j1][0], GameClient.anIntArrayArray1003[j1][anIntArray1700[j1]]);
                if (j1 == 1)
