...
public class Sprite extends DrawingArea {
	...
	public Sprite(String string, int clientWidth, int clientHeight, boolean yas) {
		try {
			Image image = GameClient.resourceLoader.getImage(string);
			if(image == null)
				return;
	...
	public Sprite(String img, int width, int height) {
		try {
			Image image;
			try {
				image = GameClient.resourceLoader.getImage(img);
			} catch(Exception e) {
				e.printStackTrace();
				return;
			}
	...
	public Sprite(String img) {
		try {
			Image image = GameClient.resourceLoader.getImage(img);
			if(image == null)
				return;
			ImageIcon sprite = new ImageIcon(image);
			myWidth = sprite.getIconWidth();
	...
	public Sprite(CacheArchive streamLoader, String s, int i) {
		...
		stream_1.pos = stream.readUnsignedWord();
	...
	public void draw(IndexedImage background, int i, int j) {
		autoUpdate();
		j += drawOffsetX;
		i += drawOffsetY;
